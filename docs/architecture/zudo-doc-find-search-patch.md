...
patch at `app/patches/@takazudo__zudo-doc@5.25.0.patch`. The nine Find/Search
hunks were re-targeted from 5.19.0 across the 5.20.0-5.25.0 range in this bump
with their bytes unchanged; a tenth hunk covering `dist/zdtp-loader.js` was
added in the same bump (see *Scope 2* below), so the patch bytes as a whole did
change. The patch SHA-256 and pnpm lock hash are both
...
`zudo-doc` 5.25.0 ships `dist/zdtp-loader.js` as a bare
`export * from "@takazudo/zdtp"`. `@takazudo/zdtp` is an **optional** peer and
...
registered by `dist/preset.js` only when `designTokenPanel` is off — which
serves the specifier as a virtual module whose body is a single `throw`.
...
This reproduces exactly what the upstream plugin would have served, keeps
`plugins: []` intact, and installs no package for a disabled feature. It is a
`zfb.config.ts`-level alias that would be the cleaner fix, but zfb 2.18.0
...
not a substitute: esbuild then passes, but the embedded V8 SSR host dies with
`load_main_es_module failed: failed to resolve "@takazudo/zdtp"`.
...
Scope 1 files rewritten by this patch between the published 5.19.0 and 5.25.0
