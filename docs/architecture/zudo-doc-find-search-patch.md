...
without changing what they do to the package; a tenth hunk covering
`dist/zdtp-loader.js` was added in the same bump (see *Scope 2* below). The
patch file itself was regenerated by `pnpm patch-commit`, which also normalized
the Find/Search hunks' empty context lines to a single space, so the file bytes
did change. The patch SHA-256 and pnpm lock hash are both
...
`zfb.config.ts`-level alias would be the cleaner fix, but zfb 2.18.0 exposes
aliasing only through the plugin `addAlias` hook — `BundleConfig`
